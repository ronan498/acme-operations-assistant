...
**2026-08-15 - caught: the audit batch, or why review agents earn their tokens**
For the final audit the AI ran several independent review passes over its own code and then adversarially verified each finding before fixing. Three catches stand out. (1) The StaticFiles mount at "/" was registered before the API routes in main.py, so POST /authz/check returned 405 from the static handler - in the Docker image only, since dev served routes first. Caught by a reviewer tracing route registration order, confirmed live, fixed by moving the mount to the end of the file. (2) The web client's stream-failure fallback re-sent the identical message through the non-streaming endpoint - if the stream died after a write tool had committed, the retry would duplicate the write. Fixed: fall back only when no tool had started; otherwise show the error. (3) The customer search interpolated user text into a LIKE pattern without escaping % and _, letting a caller turn a name lookup into a table scan match-all. All three had passed the eval suite - evals check behavior on expected paths; review checks the paths evals never walk.
