...
db-gui: ## browser DB GUI (Adminer) at http://localhost:8081, joined to the compose network
	docker run -d --rm --name acme-adminer --network acme-assistant_default \
		-e ADMINER_DEFAULT_SERVER=postgres -p 127.0.0.1:8081:8080 adminer:latest >/dev/null
	@echo "open http://localhost:8081 · User: acme · Password: acme_dev_password · Database: acme"
